...
class CrawlerLianjia():
    ...
    def __init__(self, max_page,
        ...
        self.prefix_url = self.base_url[:-1]
        ...
        self.href_house = {}
        self.create_csv()
        roots = self.get_page_url(max_page)
        for root in roots:
    ...
    def get_page_url(self, _max_page, start=1):
        page = start
        while page < _max_page:
            yield ''.join((self.base_url, 'zufang/d', str(page)))
            page += 1

    def is_root_url(self, url):
        if re.match(self.base_url + 'zufang/d', url, re.S):
            return True
        return False
    ...
    def _parse_root_per_house(self, house_box, count):
        ...
        href = self.prefix_url + house_box.xpath(
    ...
    def parse_second_etree(self, doc, href):
        ...
        house_dict['number'] = href[len(self.base_url + 'zufang/'):-5]
        ...
        house_dict['captured_at'] = datetime.now(
